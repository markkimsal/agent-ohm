    ...
    extends Mage_Dataflow_Model_Convert_Adapter_Abstract
{
    ...
    public function getEntity()
    {
        if (!$this->_entityType) {
            if (!($entityType = $this->getVar('entity_type'))
                || !(($entity = AO::getResourceSingleton($entityType)) instanceof Mage_Eav_Model_Entity_Interface)) {
    ...
    {
        try {
            $collection = AO::getResourceModel($this->getEntity().'_collection');
            $collection->load();
        } catch (Exception $e) {
            $this->addException(AO::helper('eav')->__('Problem loading the collection, aborting. Error: %s', $e->getMessage()), Varien_Convert_Exception::FATAL);
        }
