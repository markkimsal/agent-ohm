    ...
    public function checkRelation(Mage_Sales_Model_Order $order)
    {
        ...
        if (!$customer->getId()) {
            $this->_getSession()->addNotice(
                AO::helper('adminhtml')->__(' The customer doesn\'t exist in the system anymore')
            );
        }
        ...
        foreach ($order->getAllItems() as $item) {
        	if (!$productCollection->getItemById($item->getProductId())) {
        	    $this->_getSession()->addError(
        	       AO::helper('adminhtml')->__('The item %s (SKU %s) doesn\'t exist in the catalog anymore',
        	           $item->getName(),
        	           $item->getSku()
                ));
                $hasBadItems = true;
        ...
        if ($hasBadItems) {
            $this->_getSession()->addError(
                AO::helper('adminhtml')->__('Some of the ordered items don\'t exist in the catalog anymore and will be removed if you try to edit the order.')
            );
        }
        return $this;
    }
