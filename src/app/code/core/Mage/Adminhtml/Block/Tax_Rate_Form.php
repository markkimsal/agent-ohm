...
class Mage_Adminhtml_Block_Tax_Rate_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $countries = AO::getModel('adminhtml/system_config_source_country')
            ->toOptionArray();
        unset($countries[0]);

        $regionCollection = AO::getModel('directory/region')
            ->getCollection()
        ...
        $fieldset->addField('code', 'text',
            array(
                'name' => 'code',
                'label' => AO::helper('tax')->__('Tax Identifier'),
        ...
        $fieldset->addField('tax_country_id', 'select',
            array(
                'name' => 'tax_country_id',
                'label' => AO::helper('tax')->__('Country'),
                'title' => AO::helper('tax')->__('Please select Country'),
                'class' => 'required-entry',
                'required' => true,
                'values' => $countries,
                'value' => $countryId,
        ...
        $fieldset->addField('tax_region_id', 'select',
            array(
                'name' => 'tax_region_id',
                'label' => AO::helper('tax')->__('State'),
                'title' => AO::helper('tax')->__('Please select State'),
                'class' => 'required-entry',
                'required' => true,
                'values' => $regions,
        ...
        $fieldset->addField('tax_postcode', 'text',
            array(
                'name' => 'tax_postcode',
                'label' => AO::helper('tax')->__('Zip/Post Code'),
                'value' => $postcode
            )
        );
        ...
        $fieldset->addField('rate', 'text',
            array(
                'name' => "rate",
                'label' => AO::helper('tax')->__('Rate'),
                'title' => AO::helper('tax')->__('Rate'),
