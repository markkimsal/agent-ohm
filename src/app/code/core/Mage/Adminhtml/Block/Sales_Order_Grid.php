...
class Mage_Adminhtml_Block_Sales_Order_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        if (!AO::app()->isSingleStoreMode()) {
            $this->addColumn('store_id', array(
                'header'    => AO::helper('sales')->__('Purchased from (store)'),
                'index'     => 'store_id',
                'type'      => 'store',
                'store_view'=> true,
        ...
        $this->addColumn('created_at', array(
            'header' => AO::helper('sales')->__('Purchased On'),
            'index' => 'created_at',
            'type' => 'datetime',
            'width' => '100px',
        ));
        ...
        /*$this->addColumn('shipping_firstname', array(
            'header' => AO::helper('sales')->__('Ship to First name'),
            'index' => 'shipping_firstname',
        ));

        $this->addColumn('shipping_lastname', array(
            'header' => AO::helper('sales')->__('Ship to Last name'),
        ...
        $this->addColumn('base_grand_total', array(
            'header' => AO::helper('sales')->__('G.T. (Base)'),
        ...
        $this->addColumn('grand_total', array(
            'header' => AO::helper('sales')->__('G.T. (Purchased)'),
            'index' => 'grand_total',
            'type'  => 'currency',
        ...
        $this->addColumn('status', array(
            ...
            'type'  => 'options',
            'width' => '70px',
            'options' => AO::getSingleton('sales/order_config')->getStatuses(),
        ));

        if (AO::getSingleton('admin/session')->isAllowed('sales/order/actions/view')) {
        ...
        $this->addRssList('rss/order/new', AO::helper('sales')->__('New Order RSS'));
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('hold_order', array(
             'label'=> AO::helper('sales')->__('Hold'),
             'url'  => $this->getUrl('*/*/massHold'),
        ));

        $this->getMassactionBlock()->addItem('unhold_order', array(
             'label'=> AO::helper('sales')->__('Unhold'),
             'url'  => $this->getUrl('*/*/massUnhold'),
        ));

        $this->getMassactionBlock()->addItem('pdfinvoices_order', array(
             'label'=> AO::helper('sales')->__('Print Invoices'),
             'url'  => $this->getUrl('*/*/pdfinvoices'),
        ));

        $this->getMassactionBlock()->addItem('pdfshipments_order', array(
             'label'=> AO::helper('sales')->__('Print Packingslips'),
             'url'  => $this->getUrl('*/*/pdfshipments'),
        ));

        $this->getMassactionBlock()->addItem('pdfcreditmemos_order', array(
             'label'=> AO::helper('sales')->__('Print Credit Memos'),
             'url'  => $this->getUrl('*/*/pdfcreditmemos'),
        ));

        $this->getMassactionBlock()->addItem('pdfdocs_order', array(
             'label'=> AO::helper('sales')->__('Print All'),
             'url'  => $this->getUrl('*/*/pdfdocs'),
        ));

//        $statuses = AO::getSingleton('sales/order_config')->getStatuses();
//        array_unshift($statuses, array('value'=>'', 'label'=>''));
//        $this->getMassactionBlock()->addItem('change_status', array(
//             'label'=> AO::helper('sales')->__('Change Status'),
...
//                             'label' => AO::helper('sales')->__('New Status'),
//                             'values' => $statuses
//                         )
//             )
//        ));
...
//            'order'    => AO::helper('sales')->__('Orders'),
//            'invoice'  => AO::helper('sales')->__('Invoices'),
//            'shipment' => AO::helper('sales')->__('Shipments'),
//        );
...
//                             'type' => 'select',
//                             'class' => 'required-entry',
//                             'label' => AO::helper('sales')->__('Document'),
//                             'values' => $prints
//                         )
//             )
//        ));
