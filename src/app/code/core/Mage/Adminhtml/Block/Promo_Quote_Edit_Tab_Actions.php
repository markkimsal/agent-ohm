...
class Mage_Adminhtml_Block_Promo_Quote_Edit_Tab_Actions extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('simple_action', 'select', array(
            ...
            'name'      => 'simple_action',
            'options'    => array(
                'by_percent' => AO::helper('salesrule')->__('Percent of product price discount'),
                'by_fixed' => AO::helper('salesrule')->__('Fixed amount discount'),
                'cart_fixed' => AO::helper('salesrule')->__('Fixed amount discount for whole cart'),
                'buy_x_get_y' => AO::helper('salesrule')->__('Buy X get Y free (discount amount is Y)'),
            ),
        ));
        ...
        $fieldset->addField('discount_qty', 'text', array(
            'name' => 'discount_qty',
            'label' => AO::helper('salesrule')->__('Maximum Qty Discount is Applied to'),
        ));
        $model->setDiscountQty($model->getDiscountQty()*1);

        $fieldset->addField('discount_step', 'text', array(
            'name' => 'discount_step',
            'label' => AO::helper('salesrule')->__('Discount Qty Step (Buy X)'),
        ));

        $fieldset->addField('simple_free_shipping', 'select', array(
            'label'     => AO::helper('salesrule')->__('Free shipping'),
            'title'     => AO::helper('salesrule')->__('Free shipping'),
            'name'      => 'simple_free_shipping',
            'options'    => array(
                0 => AO::helper('salesrule')->__('No'),
                Mage_SalesRule_Model_Rule::FREE_SHIPPING_ITEM => AO::helper('salesrule')->__('For matching items only'),
                Mage_SalesRule_Model_Rule::FREE_SHIPPING_ADDRESS => AO::helper('salesrule')->__('For shipment with matching items'),
            ),
        ));
        ...
        $renderer = AO::getBlockSingleton('adminhtml/widget_form_renderer_fieldset')
            ->setTemplate('promo/fieldset.phtml')
            ->setNewChildUrl($this->getUrl('*/promo_quote/newActionHtml/form/rule_actions_fieldset'));

        $fieldset = $form->addFieldset('actions_fieldset', array(
            'legend'=>AO::helper('salesrule')->__('Apply the rule only to cart items matching the following conditions (leave blank for all items)')
