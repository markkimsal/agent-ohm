...
class Mage_Downloadable_DownloadController extends Mage_Core_Controller_Front_Action
{
    ...
    public function sampleAction()
    {
        ...
        $sample = AO::getModel('downloadable/sample')->load($sampleId);
        if ($sample->getId()) {
            $resource = '';
            $resourceType = '';
            ...
            try {
                ...
                $this->_getSession()->addError(AO::helper('downloadable')->__('Sorry, there was an error getting requested content. Please contact store owner.'));
            }
        }
    ...
    public function linkSampleAction()
    {
        ...
        if ($link->getId()) {
            ...
            try {
                ...
                $this->_getCustomerSession()->addError(AO::helper('downloadable')->__('Sorry, there was an error getting requested content. Please contact store owner.'));
            }
        }
        return $this->_redirectReferer();
    }
    ...
    public function linkAction()
    {
        ...
        $linkPurchasedItem = AO::getModel('downloadable/link_purchased_item')->load($id, 'link_hash');
        if (! $linkPurchasedItem->getId() ) {
            $this->_getCustomerSession()->addNotice(AO::helper('downloadable')->__("Requested link doesn't exist."));
            return $this->_redirect('*/customer/products');
        }
        if (!AO::helper('downloadable')->getIsShareable($linkPurchasedItem)) {
            $customerId = $this->_getCustomerSession()->getCustomerId();
            if (!$customerId) {
                $product = AO::getModel('catalog/product')->load($linkPurchasedItem->getProductId());
                if ($product->getId()) {
                    $notice = AO::helper('downloadable')->__(
                        'Please log in to download your product or purchase <a href="%s">%s</a>.',
                        $product->getProductUrl(), $product->getName()
                    );
                } else {
                    $notice = AO::helper('downloadable')->__('Please log in to download your product.');
                }
                $this->_getCustomerSession()->addNotice($notice);
                $this->_getCustomerSession()->authenticate($this);
                $this->_getCustomerSession()->setBeforeAuthUrl(AO::getUrl('downloadable/customer/products/'), array('_secure' => true));
                return ;
            }
            $linkPurchased = AO::getModel('downloadable/link_purchased')->load($linkPurchasedItem->getPurchasedId());
        ...
        if ($linkPurchasedItem->getStatus() == Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE
            ...
            try {
                ...
                $this->_getCustomerSession()->addError(
                    AO::helper('downloadable')->__('Sorry, there was an error getting requested content. Please contact store owner.')
                );
            }
        } elseif ($linkPurchasedItem->getStatus() == Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED) {
            $this->_getCustomerSession()->addNotice(AO::helper('downloadable')->__('Link has expired.'));
        } elseif ($linkPurchasedItem->getStatus() == Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_PENDING) {
            $this->_getCustomerSession()->addNotice(AO::helper('downloadable')->__('Link is not available.'));
        } else {
            $this->_getCustomerSession()->addError(
                AO::helper('downloadable')->__('Sorry, there was an error getting requested content. Please contact store owner.')
            );
