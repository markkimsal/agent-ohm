...
abstract class Mage_Core_Controller_Varien_Action
{
    ...
    public function preDispatch()
    {
        ...
        if (!$this->getFlag('', self::FLAG_NO_START_SESSION)) {
            ...
            if ($checkCookie && !AO::getSingleton('core/cookie')->get($namespace)) {
                $this->setFlag('', self::FLAG_NO_COOKIES_REDIRECT, true);
            }
			AO::getSingleton('core/session', array('name' => $namespace))->start();
        }

		AO::app()->loadArea($this->getLayout()->getArea());

        if ($this->getFlag('', self::FLAG_NO_COOKIES_REDIRECT) && AO::getStoreConfig('web/browser_capabilities/cookies')) {
    ...
    public function noCookiesAction()
    {
		...
		AO::dispatchEvent('controller_action_nocookies', array(
            'action'    => $this,
            'redirect'  => $redirect
        ));
