...
class Mage_Weee_Model_Total_Quote_Weee extends Mage_Sales_Model_Quote_Address_Total_Tax
{
    ...
    protected function _processItem(Mage_Sales_Model_Quote_Address $address, $item, $updateParent = false)
    {
        ...
        $attributes = AO::helper('weee')->getProductWeeeAttributes(
            $item->getProduct(),
            $address,
        ...
        foreach ($attributes as $k=>$attribute) {
            ...
            if ($item->getDiscountPercent() && AO::helper('weee')->isDiscounted($store)) {
...
//                $baseRowValue = $store->roundPrice($baseRowValue-$baseRowValueDiscount);
            ...
            if (AO::helper('weee')->isTaxable($store)) {
            ...
            $applied[] = array(
                ...
                'percent'=>null,
                'hidden'=>AO::helper('weee')->includeInSubtotal($store),
                'rates' => array(array(
                    'amount'=>$rowValue,
        ...
        AO::helper('weee')->setApplied($item, array_merge(AO::helper('weee')->getApplied($item), $productTaxes));
    ...
    {
        AO::helper('weee')->setApplied($item, array());
