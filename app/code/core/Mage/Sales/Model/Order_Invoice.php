...
class Mage_Sales_Model_Order_Invoice extends Mage_Sales_Model_Abstract
{
    ...
    {
        return AO::getSingleton('sales/order_invoice_config');
    }
    ...
    public function pay()
    {
        ...
        AO::dispatchEvent('sales_order_invoice_pay', array($this->_eventObject=>$this));
        return $this;
    }
    ...
    public function cancel()
    {
        ...
        AO::dispatchEvent('sales_order_invoice_cancel', array($this->_eventObject=>$this));
        return $this;
    }
    ...
    public function getItemsCollection()
    {
        if (empty($this->_items)) {
            $this->_items = AO::getResourceModel('sales/order_invoice_item_collection')
    ...
    public static function getStates()
    {
        if (is_null(self::$_states)) {
            self::$_states = array(
                ...
                self::STATE_PAID       => AO::helper('sales')->__('Paid'),
    ...
    public function register()
    {
        if ($this->getId()) {
            AO::throwException(
                AO::helper('sales')->__('Can not register existing invoice')
            );
        }
    ...
    public function addComment($comment, $notify=false)
    {
        if (!($comment instanceof Mage_Sales_Model_Order_Invoice_Comment)) {
            $comment = AO::getModel('sales/order_invoice_comment')
                ->setComment($comment)
    ...
    public function getCommentsCollection($reload=false)
    {
        if (is_null($this->_comments) || $reload) {
            $this->_comments = AO::getResourceModel('sales/order_invoice_comment_collection')
                ->addAttributeToSelect('*')
                ->setInvoiceFilter($this->getId())
                ->setCreatedAtOrder();
            if ($this->getId()) {
    ...
    {
        if (!AO::helper('sales')->canSendNewInvoiceEmail($this->getOrder()->getStore()->getId())) {
            return $this;
        }
    ...
    {
        if (!AO::helper('sales')->canSendInvoiceCommentEmail($this->getOrder()->getStore()->getId())) {
            return $this;
        }
