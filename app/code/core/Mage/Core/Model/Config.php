...
//		  'websites'  => 1
	...
	public function init($options=array())
	{
		 ...
		 */
		if (VPROF) Varien_Profiler::start('config/load-modules');
		...
		if (VPROF) Varien_Profiler::stop('config/load-modules');

		/**
		 * Load local configuration data
		 */
		if (VPROF) Varien_Profiler::start('config/load-local');
		...
		if (VPROF) Varien_Profiler::stop('config/load-local');
	...
	public function getSectionNode($path)
	{
		...
		if (!isset($this->_cacheLoadedSections[$sectionKey])) {
			if (VPROF) Varien_Profiler::start('mage::app::init::config::section::'.$sectionKey);
			$this->_cacheLoadedSections[$sectionKey] = $this->_loadSectionCache($sectionKey);
			if (VPROF) Varien_Profiler::stop('mage::app::init::config::section::'.$sectionKey);
		}
...
*/
	...
	protected function _loadDeclaredModules($mergeConfig)
	{
		...
		if (VPROF) Varien_Profiler::stop('config/load-modules-declaration');
		return $this;
	}
