...
class Mage_Admin_Model_Mysql4_Acl
{
    ...
    function loadAcl()
    {
        $acl = AO::getModel('admin/acl');

        AO::getSingleton('admin/config')->loadAclResources($acl);

        $roleTable = AO::getSingleton('core/resource')->getTableName('admin/role');
        $rolesArr = $this->_read->fetchAll("select * from $roleTable order by tree_level");
        $this->loadRoles($acl, $rolesArr);

        $ruleTable = AO::getSingleton('core/resource')->getTableName('admin/rule');
        $assertTable = AO::getSingleton('core/resource')->getTableName('admin/assert');
        ...
        foreach ($rolesArr as $role) {
            ...
            switch ($role['role_type']) {
                case Mage_Admin_Model_Acl::ROLE_TYPE_GROUP:
                    ...
                    $acl->addRole(AO::getModel('admin/acl_role_group', $roleId), $parent);
                    break;

                case Mage_Admin_Model_Acl::ROLE_TYPE_USER:
                    $roleId = $role['role_type'].$role['user_id'];
                    if (!$acl->hasRole($roleId)) {
                        $acl->addRole(AO::getModel('admin/acl_role_user', $roleId), $parent);
                    } else {
    ...
    function loadRules(Mage_Admin_Model_Acl $acl, array $rulesArr)
    {
    	foreach ($rulesArr as $rule) {
            ...
            $assert = null;
            if (0!=$rule['assert_id']) {
                $assertClass = AO::getSingleton('admin/config')->getAclAssert($rule['assert_type'])->getClassName();
