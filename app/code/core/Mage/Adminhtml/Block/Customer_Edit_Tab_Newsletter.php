...
class Mage_Adminhtml_Block_Customer_Edit_Tab_Newsletter extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    public function initForm()
    {
        ...
        $fieldset->addField('subscription', 'checkbox',
             array(
                    'label' => AO::helper('customer')->__('Subscribed to Newsletter?'),
                    'name'  => 'subscription'
             )
        );
        ...
        if($changedDate = $this->getStatusChangedDate()) {
             $fieldset->addField('change_status_date', 'label',
                 array(
                        'label' => $subscriber->isSubscribed() ? AO::helper('customer')->__('Last date subscribed') : AO::helper('customer')->__('Last date unsubscribed'),
                        'value' => $changedDate,
                        'bold'  => true
                 )
            );
    ...
    {
        $subscriber = AO::registry('subscriber');
