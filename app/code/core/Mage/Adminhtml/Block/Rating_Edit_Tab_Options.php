...
class Mage_Adminhtml_Block_Rating_Edit_Tab_Options extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('options_form', array('legend'=>AO::helper('rating')->__('Assigned Options')));
        ...
        if( AO::registry('rating_data') ) {
            ...
            foreach( $collection->getItems() as $item ) {
                $fieldset->addField('option_code_' . $item->getId() , 'text', array(
                                        'label'     => AO::helper('rating')->__('Option Label'),
                                        'required'  => true,
            ...
            for( $i=1;$i<=5;$i++ ) {
                $fieldset->addField('option_code_' . $i, 'text', array(
                                        'label'     => AO::helper('rating')->__('Option Title'),
                                        'required'  => true,
